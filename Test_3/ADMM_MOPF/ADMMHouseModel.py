...
class HouseholdModel():
    def __init__(self,consumerfile,pricefile,Rho):
    ...
    def createModel(self):          # https://cfwebprod.sandia.gov/cfdocs/CompResearch/docs/pyomo_bilevel_sandreport.pdf
        ...
        kW = 1 # Converts some units given in kW to W
    
        # Cost parameters
        def init_ToU(X,i):
            return self.price_data.ix[i, 'ToU'] / kW
        self.model.ToU = Param(self.model.T, initialize = init_ToU)
        #self.X.ToU = Param(self.X.T, initialize = init_ToU, mutable = True)
        ...
        ## ADMM pair and Lambda ##
        self.model.xHouse_k      = Param(self.model.T, mutable = True)
        self.model.Lambda_xHouse = Param(self.model.T, mutable = True)
        for i in self.model.T:
            self.model.xHouse_k[i] = 0; self.model.Lambda_xHouse[i] = 0
        ...
        def cost_rule_initial(model):
            return sum((self.model.ToU[i]*self.model.xPlus[i] + self.model.xMinus[i]*(-0.10))  for i in self.model.T)
        self.model.cost_initial = Objective(rule=cost_rule_initial)
        
        def cost_rule(model):
            return sum((self.model.ToU[i]*self.model.xPlus[i] + self.model.xMinus[i]*(-0.10) +
        ...
        def SoC_rule (model, i):                     # Local battery charging constraints
            return self.model.bSoC[i] == (self.model.bSoC[i-self.model.delta] + self.model.etab*self.model.xBat[i])
