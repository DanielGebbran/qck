...
# Modifications for Windows - Home Laptop:
# .loc instead of .ix
...
class HouseholdModel():
    ...
    def createModel(self):          # https://cfwebprod.sandia.gov/cfdocs/CompResearch/docs/pyomo_bilevel_sandreport.pdf
        ...
        def init_c1(model,i):
            return self.price_data.loc[i, 'c1'] / kW
        def init_c2(model,i):
            return self.price_data.loc[i, 'c2'] / kW
        ...
        def cost_rule_initial(model):
            return sum(((((self.model.xPlus[i])**2)*(self.model.c2[i]) + self.model.c1[i]*self.model.xPlus[i] + self.model.c0[i]) +
                       (self.model.xMinus[i]*(-0.00010)))  for i in self.model.T)
        self.model.cost_initial = Objective(rule=cost_rule_initial)
        '''Equivalent if initial Lambda and xH_k are zero?!'''
        def cost_rule(model):
            return sum((((self.model.xPlus[i])**2)*(self.model.c2[i]) + self.model.c1[i]*self.model.xPlus[i] + self.model.c0[i]) +
                       (self.model.xMinus[i]*(-0.00010)) +
